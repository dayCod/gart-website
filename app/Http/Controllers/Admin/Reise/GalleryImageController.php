...
class GalleryImageController extends Controller
{
    ...
    public function form(int $id): View
    {
        ...
        return view('admin.pages.reise.gallery.images.form', [
            'gallery_id' => $id,
            'pictures' => $pictures,
        ]);
    }
    ...
    public function formSave(SaveGalleryImageRequest $request, int $id): RedirectResponse
    {
        DB::beginTransaction();
        try {
            $galleryModel = Gallery::reise()->with('detailGalleries')->find($id);
            $galleryDTO = $request->dataTransferObject();

            $this->storingPictureToStorage(
                galleryModel: $galleryModel,
                galleryDTO: $galleryDTO,
                path: 'images/reise/gallery/detail'
            );
    ...
    public function destroy(int $image, int $pictureId): RedirectResponse
    {
        ...
        if ($detailGallery) {
            $filePath = public_path('storage/images/reise/gallery/detail/' . $detailGallery->image);
